...
⛔ **A `bounded` delivery cites an intent file and no design document**, because that path writes no design document at all. `docs/brainstorming/` is gitignored, so for those deliveries `intent.md` is the only record of what was decided and why that ever reaches `main`.
