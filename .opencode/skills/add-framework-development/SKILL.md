...
`scripts/build.js` (`skillStrategy.postWrite`) copies subdirs and siblings recursively, so both layouts distribute identically — the choice is purely about authoring consistency. Reference a subdoc as `.opencode/skills/NAME/references/FILE.md` (the `{{skill:}}` resolver matches greedily past `/`).
...
- **Cross-references:** `.opencode/skills/add-[name]/[file]` for files, `/add.[name]` for commands
...
- Add a `` / `` marker pair to the agent source body. Markers are **stripped at build** — `extractInjectionPoints()` records each as a content anchor in `framwork/.codeadd/injection-points.json`; the built agent files ship marker-free and injection is anchored to adjacent prose post-install.
...
- Author a per-agent fragment at `framwork/.codeadd/plugins/{plugin}/fragments/agents/{agent}.md` whose `` matches the marker.
...
#### `.opencode/commands/NAME.md` — Reference a command

```markdown

Read .opencode/commands/add.plan.md for the technical plan.
...
#### `.opencode/skills/NAME/FILE` — Reference a skill file

```markdown

Load .opencode/skills/add-backend-development/SKILL.md before implementation.
...
2. **For each resource** (command/skill/agent):
   ...
   e. **`resolveResourcePaths()`** — replaces `.opencode/commands/NAME.md` and `.opencode/skills/NAME/FILE` for each provider
...
- [ ] No raw `.codeadd/commands/` references outside code blocks (use `.opencode/commands/NAME.md`)
- [ ] No raw `.codeadd/skills/` references outside code blocks (use `.opencode/skills/NAME/FILE`)
...
## STEP 1: Load Context
Read .opencode/skills/add-backend-development/SKILL.md before implementation.
```
...
| Used raw path `.codeadd/commands/add.plan.md` | Works on Claude, breaks on Codex/Gemini/etc. | Use `.opencode/commands/add.plan.md` variable |
| Used raw path `.codeadd/skills/add-x/SKILL.md` | Works on Claude, breaks on other providers | Use `.opencode/skills/add-x/SKILL.md` variable |
...
## 8. Declaring Relationships — the `` Block
