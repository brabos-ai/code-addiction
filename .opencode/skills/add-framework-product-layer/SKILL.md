...
`add-framework-development` carries the artefact anatomies, the `` syntax and the
...
- **HTML comments are stripped at build.** Use them for source-only notes. `` and
  injection markers depend on this.
- **Never write a raw `.codeadd/` path.** Use `.opencode/commands/NAME.md` / `.opencode/skills/NAME/FILE`;
