...
**`docs/` tracking policy.** `.gitignore` ignores `docs/*`: plans, evidence, changelogs and brainstorms are working artefacts and stay local by default. The QA/UX umbrella set (topics 01–05, plans 0056–0060) is a **deliberate force-added exception** (`git add -f`, commits `b49352e` + the review-v02 fix wave) because those artefacts are the spec of record for a shipped schema change and had to survive the branch. The exception covers the umbrella's **plans, evidence files, reviews (`HANDOFF-*--review-vNN.md`) and changelogs** — a plan whose changelog or review is untracked reads as unimplemented from a fresh clone, which is the failure this policy exists to prevent. Plan set **0074** (umbrella + topics 001–004, their four evidence files) is force-added on the same grounds: it is the spec of record for a shipped convergence-gate change spanning a script, a schema, five commands and two new agents, and its evidence files carry the adversarial round that found the gate shipped four of the defects it existed to kill. Plan **0069** is tracked because the 0074 set **executed** it (T3/F20 shipped its agent and skill), so it is the spec of record for artefacts now in the product layer. Plan **0075** joins them on the same grounds: it carries the residue 0074 made visible — the consumers it migrated one side of, and the ownership questions it opened — and its Validated Decisions record two owner calls that a future reader would otherwise have to re-derive. Plan set **0078** (umbrella + three topics, each split product/internal into six plans, plus their four design docs, the build ledger and three evidence files) is force-added on the same grounds: it is the spec of record for a shipped change spanning three new scripts, a deleted skill, five product commands, three internal commands, two agents and both OpenCode adapter sets — and its ledger records **eight** conflicts between plan and tree, each with the ruling that settled it, which a future reader would otherwise have to re-derive from the diff. The **delivery-index** set (`2026-09-07T160328-PLAN--delivery-index.md`, its build ledger, its changelog `docs/changelog/2026-09-07-add-delivery-index.md`, and design docs `2026-09-07T123919-delivery-index-01`–`-04`) is force-added on the same grounds, plus one this set adds: the plan's stated method is to **reference** the design set rather than restate it, so an untracked design set turns the tracked plan into a pointer into nothing. Its ledger carries **eleven** rulings, two of which correct the plan itself — the merge pre-check needed two dots rather than three, and F13 had to land before F12 — neither of which a reader can recover from the diff. The **delivery-index-internal** set (`2026-09-07T162415-SELF-PLAN--delivery-index-internal.md`, its build ledger, and the internal design docs `2026-09-07T123919-delivery-index-05`–`-08`) is force-added by that companion self-plan, which is the owner the earlier "stay local" note deferred to. Same grounds, plus the same pointer argument: the self-plan's Context table states outright that it does not re-derive the design set, so an untracked `-05`–`-08` makes the tracked plan a pointer into nothing. It is the spec of record for a shipped change spanning a new internal command, its OpenCode adapter, a seventh `graph.js` verb — the file's first subprocess — a seventh MCP tool, four consumers with their mirrors, and this file. **The force-add retirement it designs is NOT taken here**: the trigger (`git show main:docs/delivered.jsonl | grep '"layer":"internal"' | grep '"by":"done"'`) fires for the first time only once that set's own close-out reaches `main`, and relaxing the policy before it does would delete the record of exactly this window. **Two classes under `docs/` are now tracked by `.gitignore` negation rather than by force-add**, and the distinction is the point: force-add is the mechanism that resurrected seven deleted plans, so anything meant to be durable *by rule* gets a rule, and `git add -f` stays for one-off exceptions. `!docs/delivered.jsonl` tracks the index; `!docs/changelog/` plus `!docs/changelog/**` tracks every changelog, added by this set's own close-out because a changelog is the human narrative of what shipped and the policy above already names an untracked one as reading "unimplemented from a fresh clone". **That call has been taken, and the outcome was deletion rather than tracking.** Eight changelogs written before the negation rule sat on disk unignored and uncommitted for a window; on 2026-09-08 the owner removed them instead of committing them. Everything now under `docs/changelog/` is tracked, and the directory is the record of every delivery whose changelog survived that call — not of every delivery ever made. A reader looking for the narrative of a pre-rule delivery will not find one, and that absence is a decision, not drift. `docs/plans/`, `docs/brainstorming/` and `docs/evidence/` are untouched and stay ignored. The **plain-language-rule** set (`2026-09-08T121115-PLAN--plain-language-rule.md` and its build ledger) is force-added on the established grounds: it is the spec of record for a rule that now ships into every user project's `CLAUDE.md`, spanning three skills, one command and a new test suite. Its ledger carries **three** rulings, and two of them correct the plan’s own validation spec rather than the tree — L2.3 could not assert the HTML-comment marker “in any form”, because `add-claude-md-style` quotes that exact form mid-sentence to document why it is banned; and L3.4 could not assert that update mode never *names* the derived sections, because naming them inside an explicit `⛔ DO NOT:` is the clearest instruction the command can give. Neither correction is recoverable from the diff. **No design doc is tracked alongside it** — the exploration ran inside `/add-framework--shared-brainstorm` and was cut short, so the plan carries its decisions inline and points at nothing. Plans 0001–0055 remain untracked by design — a fresh clone showing no earlier plans is expected, not drift. Do NOT "fix" this by un-ignoring `docs/`; to make another artefact durable, force-add it and say why here.
...
| `add-framework--done` | framework default | Close-out — gates, `gh` merge, index entry, cleanup |
...
| `add-framework--done` | Git branches, PRs, `docs/delivered.jsonl` |
...
- `framwork/.codeadd/scripts/build-ledger.sh` — appends one line to a feature's build ledger, creating it with its identity header when absent. **A log, not a set**: the same line twice appends twice, because de-duplicating would erase `fix round 1` followed by `fix round 2` of the same finding. Prints `LEDGER=` / `CREATED=` / `LINES=`. All three follow `converge-gates.sh`'s `KEY=VALUE` contract and its exit-2-on-CLI-misuse rule, with one deliberate departure: a write the filesystem refuses exits **1**, not 0 — the always-0 rule governs probe *results*, and a ledger line carrying a ruling that silently fails to land is the one case where exit 0 would be a lie
...
- `framwork/.codeadd/scripts/delivered.sh` — the per-project delivery index (`docs/delivered.jsonl`, tracked): `write` an entry, `read <query>`, `verify [--repair]`. Bash owns args, corpus and exit codes; an embedded node program owns JSON parse/validate/search. **The corpus is `git ls-files --cached --others --exclude-standard`, minus `docs/`, minus the index** — never a filesystem walk (which finds a stale copy of a deleted artefact and reports it alive) and never tracked-only (which reports uncommitted work dead). Both failures are what the index exists to prevent, so implementing either one reproduces the disease. Exit 2 covers caller error AND an unmet dependency, distinguished by output — `REFUSED=<ban>` vs `ERROR=node-missing`, never by a fourth code. The format is owned by `add-doc-schemas/references/delivery-index.md`, which is a **format reference, not a doc schema**, and is listed under its own H2 there for that reason
...
**Querying the graph.** `scripts/graph.js` is the engine — `impact` (transitive blast radius), `dependencies`, `neighbors`, `path`, `orphans`, `stats`, `history`, `mermaid`. `scripts/artefact-graph-mcp.js` exposes seven of them as MCP tools over stdio (hand-rolled JSON-RPC; the official SDK costs 89 transitive packages to wrap seven small functions). The CLI is the engine and MCP the wrapper, not the reverse: every provider can shell out, only some have MCP configured, and both call the same module so they cannot drift.
...
| When did this arrive, and what did it replace? | `node scripts/graph.js history <name>` |
...
**Consumers — five.** `add-framework--self-plan` STEP 2.1/2.2 derives impact and risk from it; `add-framework--sync` STEP 2 takes the ecosystem map's relationship columns from `neighbors`; `add-framework--shared-review` STEP 3.1b runs a blast-radius coverage check no single subagent can make; `add-framework--shared-brainstorm` STEP 1.2 and `add-framework--plan` (inside STEP 2.2, `--layer product`) both ask `history` whether the idea already shipped and was dropped. The docs page renders `web/public/artefact-graph.mmd`, which a test holds current against the emitted graph via the shared `DOCS_PROFILE`.
...
- Internal-layer `<!-- uses: -->` blocks **do** reach the runtime agent. `.claude/commands/*.md` are read verbatim, not built, so the block that ships to nobody in the product layer sits in the prompt in the internal one. Small, and the alternative is a second declaration mechanism for nineteen files.
