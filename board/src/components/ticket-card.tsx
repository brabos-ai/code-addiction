  ...
  return (
    <div className={cn('flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1.5', className)}>
      ...
      {showTheme && ticket.theme && (
        ...
        <span className="inline-flex min-w-0 max-w-[65%] items-center gap-1 text-xs text-muted" title={ticket.theme}>
...
export function TicketCard({ ticket, rank, from, quiet, showStatus }: Props) {
  ...
  const body = (
    <>
      ...
      <div className="mb-1.5 flex min-w-0 items-center gap-2 text-micro leading-5 text-muted">
        {showStatus && (
      ...
      <TicketMeta ticket={ticket} showId={false} className="mt-3" />
    </>
  );
  return from === '/board' ? (
