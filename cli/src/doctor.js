...
/**
 * Check if .fnd/ directory exists and is non-empty.
 * @param {string} cwd
 * @returns {{ok: boolean, exists: boolean, hasFiles: boolean}}
 */
function checkFndDir(cwd) {
  const fndDir = path.join(cwd, '.fnd');
  const exists = fs.existsSync(fndDir);
  let hasFiles = false;
  if (exists) {
    try {
      const entries = fs.readdirSync(fndDir);
...
export async function doctor(cwd) {
  ...
  const fndCheck = checkFndDir(cwd);
  ...
  const fndIcon = fndCheck.ok ? 'OK' : fndCheck.exists ? 'WARN' : 'ERROR';
  const fndStatus = fndCheck.ok ? 'present' : fndCheck.exists ? 'empty' : 'missing';
  log.info(`${fndIcon} .fnd/ directory: ${fndStatus}`);
  ...
  const allOk = nodeCheck.ok && gitCheck.ok && fndCheck.ok && manifestCheck.ok;
  ...
  if (allOk) {
    ...
    if (!fndCheck.ok) issues.push('.fnd/ directory missing or empty');
