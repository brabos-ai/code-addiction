...
describe('config show command', () => {
  it('exits 0 when manifest exists', async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        hashes: { '.fnd/commands/fnd.md': 'abc123' },
      }),
      'utf8'
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        files: [
          ...
          '.kilocode/commands/fnd.md',
          '.codex/commands/fnd.md',
          '.opencode/commands/fnd.md'
        ],
        hashes: {
  ...
  it('displays correct file count with many files', async () => {
    ...
    const files = Array.from({ length: 100 }, (_, i) => `.fnd/file${i}.txt`);
    const hashes = {};
    files.forEach((f, i) => {
      hashes[f] = `hash${i}`;
    });
