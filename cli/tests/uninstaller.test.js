...
describe('readManifest', () => {
  ...
  it('parses a valid manifest', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.pff', 'manifest.json'),
...
describe('uninstall', () => {
  it('throws if manifest does not exist', async () => {
    ...
    const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pff-empty-'));
  ...
  it('removes files listed in manifest (--force)', async () => {
    ...
    // Create fake PFF files
    const pffDir = path.join(tmpDir, '.pff');
    const commandsDir = path.join(pffDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    fs.writeFileSync(path.join(commandsDir, 'pff.md'), '# pff');
    ...
    expect(fs.existsSync(path.join(commandsDir, 'pff.md'))).toBe(false);
    expect(fs.existsSync(path.join(pffDir, 'manifest.json'))).toBe(false);
  });
});
