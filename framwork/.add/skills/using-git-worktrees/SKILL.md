...
2. Execute os comandos de desenvolvimento lá:
   ...
   - `/add-done` - para finalizar

### Neste VSCode (atual):
- Você pode continuar trabalhando em outras coisas
- A worktree é independente - não afeta este workspace
- Quando terminar a feature, a branch será mergeada via `/add-done` no outro VSCode
...
### No novo VSCode:
1. O Claude Code terá contexto isolado da worktree
2. Execute `/plan`, `/dev`, `/autopilot`, ou `/add-done` lá
...
- `/add-done` → Finaliza e faz merge (executado no VSCode da worktree)
