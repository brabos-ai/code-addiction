<!-- AUTO-GENERATED - DO NOT EDIT. Source: framwork/.codeadd/commands/add.ship.md -->
---
name: add.ship
...
2. IF NOT EXISTS:
   → "⛔ Review not executed. Run /add.check before /add.ship."
   → BLOCKED
...
5. IF Overall = ❌ BLOCKED:
   ...
   → "⛔ Review found problems. Fix and re-run /add.check."
   → BLOCKED
   → IF --force: proceed to 4A.1, register in changelog:
...
Options:
1. Continue development: /add.build feature ${NEXT}
2. Force merge anyway: /add.ship --force
```
...
After merge, run: /add.ship (will detect merged PR and cleanup)
```
