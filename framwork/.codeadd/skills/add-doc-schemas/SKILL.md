...
description: Use when an ADD command generates or updates a documentation asset — single source of truth for universal doc rules, depth floors, output-length doctrine, stable IDs, {{doc:ID}} refs, and validation gate. Schemas live in references/<category>.md by purpose. Load before any doc write.
---
...
**Agent-generated documentation has no character or word limit.** Numeric advisories (`<200 words`, `~100-150 words`, `<500 chars`) are **prohibited** in any rule that constrains agent output. Length is governed by **depth floors** (minimum content per section) and **density** (every sentence is fact, decision, constraint, link, or signal — never filler).
...
Compression exists to enable grep-as-retrieval and JIT loading — **not** to save tokens. If density and completeness conflict, completeness wins: split the section, link out, or accept a longer chunk. Never drop load-bearing information to "fit".

**Inherently-short genres** (TL;DR, frontmatter `description`, slugs) are described by *form*, not by number. TL;DR is brief by genre — must answer what the doc is, why it exists, and the headline decision/outcome. Stop when those three are covered. No number applies.

**Platform-imposed limits stay.** Anthropic SDK metadata (`name ≤64 chars`, `description ≤1024 chars`), UI input validators in vendor sample code, and identifier-shape rules (slugs, branch names) are external constraints — not framework opinions on documentation density.
...
All generated docs carry the canonical frontmatter defined in Universal Document Requirements below (`id`, `type`, `created`, `updated`, `related`). Optional `by:` signals the editor (agent name or `human`). No separate trailing metadata block is required — the frontmatter IS the metadata.
...
5. **Non-redundancy / density.** For every sentence, ask: does it repeat another section, restate the heading, or paraphrase the source? If yes: delete. Every sentence must be a fact, decision, constraint, link, or observable signal — never filler. Density is enforced here; numeric length caps are prohibited.
...
- **No inline examples of full docs.** Reference by `{{doc:}}` to a canonical example in `docs/examples/` (future) when the example exceeds a small snippet.
