...
- **Depth floor:**
  ...
  - **Fix Routing** — the **union** of every in-scope `qa-validation-NNN.md`'s Fix Routing rows, where `qa-pipeline` produced any, plus the code-review, build-failure and validation-gate findings. With the feature off there are no per-scope reports and the union is the other three classes. Columns: `Scope | ID | Severity | Area | Route | File | Symptom | Blocked by`. Ordered by severity precedence, then `database → backend → frontend → e2e`. Every row is scope-qualified and there is **no dedup rule** — two subfeatures reporting one symptom are two rows with two fix sites, and merging them loses a fix site.
