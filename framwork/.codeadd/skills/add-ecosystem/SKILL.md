...
| add.plan-to-ready | Bounded convergence loop — plans, then loops build ⇄ review at most 3 iterations per invocation against the `/add.done` gates evaluated in dry-run. Dispatches named leaf agents at depth 1, never commands. Reports CONVERGED / CAP_REACHED / BLOCKED and returns control; the merge stays human | add-doc-schemas, add-id-convention, add-tasks-checklist, add-ecosystem |
