...
4. Parse PROJECT_PATHS from script output and read ALL listed files
   - Example output: PROJECT_PATHS:.fnd/project/SERVER.md,.fnd/project/ADMIN.md,.fnd/project/DATABASE.md
...
prompt: |
  ...
  - .fnd/skills/backend-development/SKILL.md (if backend scope)
  - .fnd/skills/database-development/SKILL.md (if database scope)
  - .fnd/skills/frontend-development/SKILL.md (if frontend scope)
...
prompt: |
  ...
  - .fnd/skills/database-development/SKILL.md
  ...
  - CHOICE: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "database" '"decision":"[approach]","reason":"[why]","alternatives":["[alt]"]'`
  - PIVOT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "database" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
  - RESULT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "database" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]"]'`
...
prompt: |
  ...
  ## SKILLS
  MANDATORY - Read BEFORE implementing:
  - .fnd/skills/backend-development/SKILL.md
  ...
  ## DECISION LOGGING (MANDATORY — PRD0031)
  Log to `docs/features/${FEATURE_ID}/decisions.jsonl` using log script:
  - CHOICE: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "backend" '"decision":"[approach]","reason":"[why]","alternatives":["[alt]"]'`
  - PIVOT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "backend" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
  - RESULT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "backend" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]"]'`
...
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "add" "/autopilot" '"slug":"<FEATURE_SLUG>","what":"<WHAT max 60 chars>","files":["<list from Decision Log>"]'
```
...
**Update epic.md subfeature status to `in_progress` (will move to `done` after `/fnd-done`).**
...
prompt: |
  ...
  - .fnd/skills/code-review/SKILL.md (CODE validation)
  - .fnd/skills/delivery-validation/SKILL.md (PRODUCT validation)

  REFERENCE:
  - Backend patterns: .fnd/skills/backend-development/SKILL.md
  - Database patterns: .fnd/skills/database-development/SKILL.md
  - Frontend patterns: .fnd/skills/frontend-development/SKILL.md
  - Security: .fnd/skills/security-audit/SKILL.md
  ...
  - review.md MUST be created (merge prerequisite for /fnd-done)
...
4. Run /fnd-done (reads review.md automatically)
```
...
3. When ready: /autopilot feature ${N+1}
   OR: /fnd-dev feature ${N+1} (for manual control)
4. When all Epic features complete: /fnd-done
```
