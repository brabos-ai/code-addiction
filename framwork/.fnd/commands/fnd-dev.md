...
```bash
bash .fnd/scripts/feature-status.sh
```
...
**IF `.fnd/project/decisions.jsonl` exists:**
1. READ file
...
- ⛔ DO NOT: Start implementation
- ⛔ DO: Inform user all subfeatures complete → run `/fnd-done`

### 2B: Legacy Feature Flag ("feature N" passed)

**Syntax:** `/fnd-dev feature N` or `/fnd-dev feature 1`
...
# Example: PROJECT_PATHS:.fnd/project/SERVER.md,.fnd/project/ADMIN.md,.fnd/project/DATABASE.md
...
**If ITERATIONS output exists from script:** Previous /fnd-dev sessions context - avoid repeating fixes.
...
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "[area]" '"decision":"[approach chosen]","reason":"[why]","alternatives":["[alt1]"]'
```
- **On pivot** (changed approach):
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "[area]" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N],"error":"[if any]"'
```
- **On success** (result):
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "[area]" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]","[f2]"]'
```
...
1. Run: bash .fnd/scripts/feature-status.sh
...
- Backend API: .fnd/skills/backend-development/SKILL.md
- Database: .fnd/skills/database-development/SKILL.md
- Frontend: .fnd/skills/frontend-development/SKILL.md (auto-loads ux-design if no design.md)
...
- CHOICE: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "[area]" '"decision":"[what]","reason":"[why]","alternatives":["[alt1]"]'`
- PIVOT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "[area]" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
- RESULT: `bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "[area]" '"decision":"[final choice]","status":"success","attempt":[N],"files":["[f1]"]'`
...
- **MANDATORY:** Load skill `.fnd/skills/frontend-development/SKILL.md` first
...
```json
{"backend":".fnd/skills/backend-development/SKILL.md (RESTful,IoC,DTOs,CQRS,Multi-tenancy)","database":".fnd/skills/database-development/SKILL.md (Entities,Migrations,Kysely,Repositories)","frontend":".fnd/skills/frontend-development/SKILL.md (Types,Hooks,State,API,Forms,Routing+auto-loads ux-design)"}
```
...
BEFORE writing code, read: .fnd/skills/[area]-development/SKILL.md
...
1. Read: .fnd/skills/frontend-development/SKILL.md (FIRST - handles types, hooks, state, API)
2. If NO design.md: Load .fnd/skills/ux-design/SKILL.md for SaaS UX patterns
...
- shadcn: .fnd/skills/ux-design/shadcn-docs.md
- Tailwind: .fnd/skills/ux-design/tailwind-v3-docs.md
- Motion: .fnd/skills/ux-design/motion-dev-docs.md
- Charts: .fnd/skills/ux-design/recharts-docs.md
- Tables: .fnd/skills/ux-design/tanstack-table-docs.md
- Query: .fnd/skills/ux-design/tanstack-query-docs.md
```
...
1. FIRST, load the UX design skill: Read `.fnd/skills/ux-design/SKILL.md`
2. Follow ALL patterns from the skill (mobile-first, shadcn, Tailwind v3, Motion, etc.)
3. For component fixes: Grep pattern="[component]" path=".fnd/skills/ux-design/shadcn-docs.md"
4. For styling fixes: Grep pattern="[utility]" path=".fnd/skills/ux-design/tailwind-v3-docs.md"
5. For animation fixes: Grep pattern="[pattern]" path=".fnd/skills/ux-design/motion-dev-docs.md"
...
prompt: |
  ...
  1. Run: bash .fnd/scripts/feature-status.sh
  2. Parse FEATURE_ID from output
  3. Read skill: .fnd/skills/${AREA}-development/SKILL.md

  ## SKILLS
  MANDATORY - Read BEFORE validating:
  - .fnd/skills/${AREA}-development/SKILL.md (contains Validation Checklist)
...
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "<TYPE>" "/dev" '"slug":"<SLUG>","what":"<WHAT max 60 chars>","files":["<file1>","<file2>"]'
```

**IF epic subfeature (HAS_EPIC=true), add `"sf"` field:**
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "<TYPE>" "/dev" '"slug":"<SLUG>","what":"<WHAT>","files":["<f1>"],"sf":"${EPIC_CURRENT_SF}"'
```
...
4. When approved, run `/fnd-done` to merge
```

### Feature Completion (when executing `/fnd-dev feature N` in Epic)
...
3. When ready, execute: `/fnd-dev feature ${N+1}`
4. Or if all Epic features complete: `/fnd-done`
```
...
```bash
# User executes:
/fnd-dev the save button is not working, error 500
