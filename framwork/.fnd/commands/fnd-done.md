...
IF BRANCH_TYPE = feature AND REVIEW_MD NOT FOUND AND NOT --force:
  ...
  ⛔ DO: Inform user to run /fnd-review first

IF BRANCH_TYPE = feature AND REVIEW_MD OVERALL = BLOCKED AND NOT --force:
  ⛔ DO NOT USE: Write to create changelog.md
  ⛔ DO NOT USE: Bash for done.sh --merge
  ⛔ DO: Show blocked gates and request /fnd-review re-run
...
2. IF NOT EXISTS:
   → "⛔ Review not executed. Run /fnd-review before /fnd-done."
   → BLOCKED
...
5. IF Overall = ❌ BLOCKED:
   ...
   → "⛔ Review found problems. Fix and re-run /fnd-review."
   → BLOCKED
   → IF --force: proceed to 4A.1, register in changelog:
...
Options:
1. Continue: /fnd-dev (implements next subfeature)
2. Force merge anyway: /fnd-done --force
```
...
Options:
1. Continue development: /fnd-dev feature ${NEXT}
2. Force merge anyway: /fnd-done --force
```
...
3. Force merge anyway: /fnd-done --force
```
...
CENTRAL_DECISIONS = .fnd/project/decisions.jsonl
...
```bash
bash .fnd/scripts/done.sh --merge
```
