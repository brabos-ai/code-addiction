# fnd-test — Automated Test Generation
...
```json
{
  ...
  "patterns": {"if_exists": ".fnd/project/*.md", "action": "READ for project context"}
}
```
...
| Input | Mode | Scope |
|-------|------|-------|
| `fnd-test` (no args) | `diff` | Files changed in git diff |
| `fnd-test feature N` | `feature` | Entire feature N scope |
| `fnd-test <path>` | `path` | Specific directory/file |

SET `SCOPE_MODE` = determined mode.

### 2.2 Detect Existing Contract Tests (PRD0001 — TDD)

**BEFORE generating new tests, check for contract tests from `/fnd-plan`:**

```bash
# Find existing contract test files (generated by test-spec subagent via /fnd-dev)
...
5. INFORM user: "Detected [N] contract test files from /fnd-plan. Will focus on gaps and edge cases."
...
IF no changed files:
  INFORM user: "No changed files detected. Specify a path or feature: `fnd-test src/modules/auth` or `fnd-test feature 1`"
  ⛔ STOP.

**IF MODE = feature:**
1. RUN: `bash .fnd/scripts/feature-status.sh` (if exists)
...
1. READ .fnd/skills/backend-development/SKILL.md (if exists) for project patterns
2. READ .fnd/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .fnd/skills/frontend-development/SKILL.md (if exists) for project patterns
2. READ .fnd/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .fnd/skills/backend-development/SKILL.md (if exists)
2. READ .fnd/project/*.md (if exists)
...
IF `.fnd/scripts/log-iteration.sh` exists:
```bash
bash .fnd/scripts/log-iteration.sh "test" "fnd-test" "Generated tests — [CURRENT_COVERAGE]% coverage" "[ALL_TEST_FILES comma-separated]"
```
...
- Regenerate contract tests that already exist from /fnd-plan (detect and skip)
