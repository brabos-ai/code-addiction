...
**STEPS IN ORDER:**
```
STEP 1: Detect templates       → fnd template status
STEP 2: Execute updates        → fnd template update <slug> for each
...
IF TEMPLATES NOT DETECTED:
  ⛔ DO NOT USE: Bash for fnd template update
  ⛔ DO NOT: Update templates
  ✅ DO: Execute fnd template status FIRST
...
| Authentication error | Inform `fnd login` and STOP |

**IF no templates installed:**
```
## FND Update

No FND templates installed in this project.

To install: `fnd template install <slug>`
To list available: `fnd template list`
```
→ STOP HERE.
...
**ANALYZE the message returned by `fnd template update`:**
...
To keep: `git add . && git commit -m "chore: update fnd templates"`
...
### DETECTION
- [ ] STEP 1: `fnd template status` executed
- [ ] Templates identified (or informed that there are none)

### UPDATE
- [ ] STEP 2: `fnd template update` executed for each template
...
| Error | Action |
|-------|--------|
| `fnd` not found | Inform: "FND CLI not installed. Execute: npm i -g @brabos-ai/fnd-cli" |
| Not authenticated | Inform: "Execute `fnd login` to authenticate" |
...
- Ignore the message returned by fnd template update
