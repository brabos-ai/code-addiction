# FND - Intelligent Ecosystem Gateway

> **LANG:** Respond in user's native language (detect from input). Tech terms always in English. Documents in user's language.

Entry point for the fnd-pro ecosystem. Answers questions, guides flow, suggests next command.
...
STEP 1: Check onboarding         → IF docs/owner.md does not exist → suggest /fnd-init
...
```bash
cat .fnd/skills/fnd-ecosystem-map/SKILL.md
```

This file contains:
- All fnd-pro commands with purpose and skills they load
...
**Use:** `/fnd-init` to run onboarding (5-10 min)
...
```bash
cat .claude/commands/fnd-[command].md
```
...
```bash
cat .fnd/skills/dev-environment-setup/SKILL.md
```
...
|------------------|-------------------|-----------|
| No docs/owner.md | `/fnd-init` | Project onboarding |
...
| Feature reviewed | `/fnd-done` | Finalize and generate changelog |
| Epic with pending sub-features | `/fnd-dev feature N` | Next sub-feature |
...
Or use `/fnd [question]` for more guidance.
```
