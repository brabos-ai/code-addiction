...
if [ "$CREATE_PR" = false ]; then
    ...
    echo "COMMAND=bash .fnd/scripts/feature-pr.sh --create-pr"
    exit 0
fi
...
Generated with FND by https://brabos.ai

Co-Authored-By: FND <noreply@brabos.ai>"
    echo "COMMIT=OK"
fi
...
echo ""
echo "NEXT_STEP=After PR is merged, run: bash .fnd/scripts/feature-pr.sh --confirm-merge"
