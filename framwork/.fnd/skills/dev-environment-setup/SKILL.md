...
description: Use when user needs to set up bash/git/jq/gh CLI to run fnd-pro scripts, environment tools are missing, or VS Code terminal is not WSL — detects OS, diagnoses gaps silently, confirms with user, installs missing tools in correct order, and merges WSL profile into .vscode/settings.json on Windows
---
