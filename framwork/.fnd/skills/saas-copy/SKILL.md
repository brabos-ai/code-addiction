...
**Não use para:** Executar workflow (use comando `/fnd-copy`)

**Relação:**
- `saas-copy` (skill) = Conhecimento, frameworks, templates
- `/fnd-copy` (comando) = Execução do workflow + geração de arquivos
...
- Para executar workflow completo → use `/fnd-copy`
...
O comando `/fnd-copy` usa estas fontes para análise automática:
...
Perguntas que o comando `/fnd-copy` faz ao usuário:
