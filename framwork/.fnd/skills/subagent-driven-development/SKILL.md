...
Dispatch review subagent with this prompt:
    ...
    ## SKILLS
    - .fnd/skills/code-review/SKILL.md
...
**Reference scripts:**
- `bash .fnd/scripts/feature-status.sh` - Get feature context
- `bash .fnd/scripts/architecture-discover.sh` - Codebase structure overview

**Skills to compose:**
- Backend: `.fnd/skills/backend-development/SKILL.md`
- Database: `.fnd/skills/database-development/SKILL.md`
- Frontend: `.fnd/skills/frontend-development/SKILL.md` + `.fnd/skills/ux-design/SKILL.md`
- Review: `.fnd/skills/code-review/SKILL.md`
...
```bash
bash .fnd/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "<type>" "<agent>" '"decision":"[what]","reason":"[why]"'
```

### Central File (consolidated by /fnd-done)

`.fnd/project/decisions.jsonl` — all project decisions, consolidated at feature completion.
...
After plan.md is created and validated (STEP 9.4 of /fnd-plan).
...
### TASKS MODE in /fnd-dev

When `HAS_TASKS=true`, /fnd-dev executes by tasks:
