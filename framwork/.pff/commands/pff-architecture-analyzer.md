...
```json
{"gates":["discovery_script_run","apps_classified","all_analyzers_complete","context_files_updated"],"order":["discovery_script","classify_apps","dispatch_specialists","consolidate_reports","update_claude_md","copy_context_files","report_cleanup"],"specialists":{"backend":"backend-analyzer.md","frontend":"frontend-analyzer.md","database":"database-analyzer.md","code_quality":"code-quality-analyzer.md","generic":"GenericAppTemplate"},"outputs":{".pff/project":"APP-*.md|LIB-*.md","docs":"code-quality-review.md","root":"CLAUDE.md|AGENTS.md|GEMINI.md"}}
```
...
IF SKILL.md NOT READ:
  ...
  ⛔ DO: Read .pff/skills/architecture-discovery/SKILL.md FIRST
...
**READ:**
```bash
Read: .pff/skills/architecture-discovery/SKILL.md
```
...
**Execute:**
```bash
bash .pff/scripts/architecture-discover.sh
```

**Verify output:**
```bash
ls .pff/temp/architecture-discovery.md
```
...
4. WRITE findings to `.pff/temp/architecture-discovery.md`
...
## SELF-BOOTSTRAP
Read: .pff/skills/architecture-discovery/[TYPE]-analyzer.md
Follow ALL instructions in that file.

## CONTEXT
Read: .pff/temp/architecture-discovery.md

## TASK
1. Analyze ONLY the app at: [APP_PATH]
2. Follow skill instructions for [TYPE] patterns
3. WRITE file to .pff/project/[PREFIX]-[DIR_NAME].md
...
## REPORT FORMAT
Return summary:
- FILE_WRITTEN: .pff/project/[PREFIX]-[DIR_NAME].md (or NONE)
- TYPE: [TYPE]
- FRAMEWORKS: [discovered]
- PATTERNS_FOUND: [list]
```

- **Output:** Write `.pff/project/[PREFIX]-[DIR_NAME].md`
...
4. WRITE file to .pff/project/[PREFIX]-[DIR_NAME].md
   - PREFIX = APP (if in apps/) or LIB (if in libs/packages/)
   - DIR_NAME = actual directory name in UPPERCASE
...
## SELF-BOOTSTRAP
Read: .pff/skills/architecture-discovery/database-analyzer.md
Follow ALL instructions in that file.
...
2. If database found in a lib (e.g., libs/database): WRITE file to .pff/project/LIB-[DIR_NAME].md
3. If database is cross-app without dedicated lib: WRITE file to .pff/project/LIB-DATABASE.md
...
## REPORT FORMAT
Return summary:
- FILE_WRITTEN: .pff/project/LIB-[DIR_NAME].md (or NONE)
- STACK: [engine + ORM + migrations]
- PATTERNS_FOUND: [list]
```

- **Output:** Write `.pff/project/LIB-[DIR_NAME].md`
...
## SELF-BOOTSTRAP
Read: .pff/skills/architecture-discovery/code-quality-analyzer.md
Follow ALL instructions in that file.
...
- [ ] All `.pff/project/*.md` files written by specialist agents
- [ ] `docs/code-quality-review.md` written by code quality agent

**COLLECT reports:**
- Files written (`.pff/project/*.md`)
...
## INPUTS TO READ
1. .pff/temp/architecture-discovery.md
2. ALL files in .pff/project/*.md (list dynamically)
...
3. **## Implementation Patterns** section (list all .pff/project/*.md)

## OUTPUT FORMAT
- JSON minified one-line
- Max 10 words per description
- List ALL .pff/project/*.md dynamically

## REPORT FORMAT
Return summary:
- CLAUDE_MD_UPDATED: YES
- SECTIONS_UPDATED: [list]
- PATTERNS_REFERENCED: [list .pff/project/*.md files]
```
...
|-----|------|------------|--------|
| [path] | [type] | [yes/generic] | .pff/project/[PREFIX]-[DIR_NAME].md |

**Files Generated:**
- [list all .pff/project/*.md]
...
2. Review .pff/project/*.md for implementation patterns
...
**Execute:**
```bash
rm .pff/temp/architecture-discovery.md 2>/dev/null || true
```
