...
4. Parse PROJECT_PATHS from script output and read ALL listed files
   - Example output: PROJECT_PATHS:.pff/project/SERVER.md,.pff/project/ADMIN.md,.pff/project/DATABASE.md
...
prompt: |
  ...
  - .pff/skills/backend-development/SKILL.md (if backend scope)
  - .pff/skills/database-development/SKILL.md (if database scope)
  - .pff/skills/frontend-development/SKILL.md (if frontend scope)
...
prompt: |
  ...
  - .pff/skills/database-development/SKILL.md
  ...
  - CHOICE: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "database" '"decision":"[approach]","reason":"[why]","alternatives":["[alt]"]'`
  - PIVOT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "database" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
  - RESULT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "database" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]"]'`
...
prompt: |
  ...
  ## SKILLS
  MANDATORY - Read BEFORE implementing:
  - .pff/skills/backend-development/SKILL.md
  ...
  ## DECISION LOGGING (MANDATORY — PRD0031)
  Log to `docs/features/${FEATURE_ID}/decisions.jsonl` using log script:
  - CHOICE: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "backend" '"decision":"[approach]","reason":"[why]","alternatives":["[alt]"]'`
  - PIVOT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "backend" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
  - RESULT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "backend" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]"]'`
...
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "add" "/autopilot" '"slug":"<FEATURE_SLUG>","what":"<WHAT max 60 chars>","files":["<list from Decision Log>"]'
```
...
**Update epic.md subfeature status to `in_progress` (will move to `done` after `/pff-done`).**
...
prompt: |
  ...
  - .pff/skills/code-review/SKILL.md (CODE validation)
  - .pff/skills/delivery-validation/SKILL.md (PRODUCT validation)

  REFERENCE:
  - Backend patterns: .pff/skills/backend-development/SKILL.md
  - Database patterns: .pff/skills/database-development/SKILL.md
  - Frontend patterns: .pff/skills/frontend-development/SKILL.md
  - Security: .pff/skills/security-audit/SKILL.md
  ...
  - review.md MUST be created (merge prerequisite for /pff-done)
...
4. Run /pff-done (reads review.md automatically)
```
...
3. When ready: /autopilot feature ${N+1}
   OR: /pff-dev feature ${N+1} (for manual control)
4. When all Epic features complete: /pff-done
```
