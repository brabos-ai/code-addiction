...
```bash
bash .pff/scripts/feature-status.sh
```
...
**IF `.pff/project/decisions.jsonl` exists:**
1. READ file
...
- ⛔ DO NOT: Start implementation
- ⛔ DO: Inform user all subfeatures complete → run `/pff-done`

### 2B: Legacy Feature Flag ("feature N" passed)

**Syntax:** `/pff-dev feature N` or `/pff-dev feature 1`
...
# Example: PROJECT_PATHS:.pff/project/SERVER.md,.pff/project/ADMIN.md,.pff/project/DATABASE.md
...
**If ITERATIONS output exists from script:** Previous /pff-dev sessions context - avoid repeating fixes.
...
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "[area]" '"decision":"[approach chosen]","reason":"[why]","alternatives":["[alt1]"]'
```
- **On pivot** (changed approach):
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "[area]" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N],"error":"[if any]"'
```
- **On success** (result):
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "[area]" '"decision":"[final]","status":"success","attempt":[N],"files":["[f1]","[f2]"]'
```
...
1. Run: bash .pff/scripts/feature-status.sh
...
- Backend API: .pff/skills/backend-development/SKILL.md
- Database: .pff/skills/database-development/SKILL.md
- Frontend: .pff/skills/frontend-development/SKILL.md (auto-loads ux-design if no design.md)
...
- CHOICE: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "choice" "[area]" '"decision":"[what]","reason":"[why]","alternatives":["[alt1]"]'`
- PIVOT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "pivot" "[area]" '"from":"[old]","decision":"[new]","reason":"[why]","attempt":[N]'`
- RESULT: `bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "result" "[area]" '"decision":"[final choice]","status":"success","attempt":[N],"files":["[f1]"]'`
...
- **MANDATORY:** Load skill `.pff/skills/frontend-development/SKILL.md` first
...
```json
{"backend":".pff/skills/backend-development/SKILL.md (RESTful,IoC,DTOs,CQRS,Multi-tenancy)","database":".pff/skills/database-development/SKILL.md (Entities,Migrations,Kysely,Repositories)","frontend":".pff/skills/frontend-development/SKILL.md (Types,Hooks,State,API,Forms,Routing+auto-loads ux-design)"}
```
...
BEFORE writing code, read: .pff/skills/[area]-development/SKILL.md
...
1. Read: .pff/skills/frontend-development/SKILL.md (FIRST - handles types, hooks, state, API)
2. If NO design.md: Load .pff/skills/ux-design/SKILL.md for SaaS UX patterns
...
- shadcn: .pff/skills/ux-design/shadcn-docs.md
- Tailwind: .pff/skills/ux-design/tailwind-v3-docs.md
- Motion: .pff/skills/ux-design/motion-dev-docs.md
- Charts: .pff/skills/ux-design/recharts-docs.md
- Tables: .pff/skills/ux-design/tanstack-table-docs.md
- Query: .pff/skills/ux-design/tanstack-query-docs.md
```
...
1. FIRST, load the UX design skill: Read `.pff/skills/ux-design/SKILL.md`
2. Follow ALL patterns from the skill (mobile-first, shadcn, Tailwind v3, Motion, etc.)
3. For component fixes: Grep pattern="[component]" path=".pff/skills/ux-design/shadcn-docs.md"
4. For styling fixes: Grep pattern="[utility]" path=".pff/skills/ux-design/tailwind-v3-docs.md"
5. For animation fixes: Grep pattern="[pattern]" path=".pff/skills/ux-design/motion-dev-docs.md"
...
prompt: |
  ...
  1. Run: bash .pff/scripts/feature-status.sh
  2. Parse FEATURE_ID from output
  3. Read skill: .pff/skills/${AREA}-development/SKILL.md

  ## SKILLS
  MANDATORY - Read BEFORE validating:
  - .pff/skills/${AREA}-development/SKILL.md (contains Validation Checklist)
...
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "<TYPE>" "/dev" '"slug":"<SLUG>","what":"<WHAT max 60 chars>","files":["<file1>","<file2>"]'
```

**IF epic subfeature (HAS_EPIC=true), add `"sf"` field:**
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "<TYPE>" "/dev" '"slug":"<SLUG>","what":"<WHAT>","files":["<f1>"],"sf":"${EPIC_CURRENT_SF}"'
```
...
4. When approved, run `/pff-done` to merge
```

### Feature Completion (when executing `/pff-dev feature N` in Epic)
...
3. When ready, execute: `/pff-dev feature ${N+1}`
4. Or if all Epic features complete: `/pff-done`
```
...
```bash
# User executes:
/pff-dev the save button is not working, error 500
