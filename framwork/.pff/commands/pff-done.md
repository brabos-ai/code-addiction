...
IF BRANCH_TYPE = feature AND REVIEW_MD OVERALL = BLOCKED AND NOT --force:
  ⛔ DO NOT USE: Write to create changelog.md
  ⛔ DO NOT USE: Bash for done.sh --merge
  ⛔ DO: Show blocked gates and request /pff-review re-run
...
2. IF NOT EXISTS:
   → "⛔ Review not executed. Run /pff-review before /pff-done."
   → BLOCKED
...
5. IF Overall = ❌ BLOCKED:
   ...
   → "⛔ Review found problems. Fix and re-run /pff-review."
   → BLOCKED
   → IF --force: proceed to 4A.1, register in changelog:
...
Options:
1. Continue: /pff-dev (implements next subfeature)
2. Force merge anyway: /pff-done --force
```
...
Options:
1. Continue development: /pff-dev feature ${NEXT}
2. Force merge anyway: /pff-done --force
```
...
3. Force merge anyway: /pff-done --force
```
...
CENTRAL_DECISIONS = .pff/project/decisions.jsonl
...
```bash
bash .pff/scripts/done.sh --merge
```
