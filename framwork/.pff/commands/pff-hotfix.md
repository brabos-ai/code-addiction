...
## Spec
```json
{"gates":["path_decided","template_read","branch_created","docs_read","root_cause_confirmed"],"investigate":{"order":["RECENT_CHANGELOGS","docs/features/*/changelog.md","docs/features/*/about.md","code"]},"patterns":{"if_exists":".pff/projects/*-patterns.md","action":"READ before fix"},"modes":{"feature_fix":"fix/F[XXXX]-[name]","standalone":"fix/H[XXXX]-[name]"},"template":".pff/templates/hotfix-template.md"}
```
...
STEP 11: Inform user            → AWAITING /pff-done
```
...
IF TEMPLATE NOT READ:
  ⛔ DO NOT USE: Write to create hotfix doc
  ✅ DO: Read .pff/templates/hotfix-template.md FIRST
...
⛔ **GATE:** READ `.pff/templates/hotfix-template.md` BEFORE creating doc.

### 4.1 Read Template (MANDATORY)

```
Read .pff/templates/hotfix-template.md
```
...
**FRONTEND FIXES:**
If bug in frontend:
1. READ `.pff/skills/ux-design/SKILL.md`
...
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/iterations.jsonl" "fix" "/hotfix" '"slug":"<SLUG>","what":"<WHAT max 60 chars>","files":["<file1>","<file2>"]'
```

**Standalone hotfix (PATH B):**
```bash
bash .pff/scripts/log-jsonl.sh "docs/hotfixes/iterations.jsonl" "fix" "/hotfix" '"slug":"<SLUG>","what":"<WHAT max 60 chars>","files":["<file1>","<file2>"]'
```
...
⛔ **DO NOT commit** - leave for `/pff-done`
...
1. Review changes
2. Test fix
3. Execute `/pff-done` to finalize
```
...
- [ ] STEP 11: User informed, awaiting `/pff-done`
...
- Commit changes (leave for /pff-done)
