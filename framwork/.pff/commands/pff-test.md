# pff-test — Automated Test Generation
...
```json
{
  ...
  "patterns": {"if_exists": ".pff/project/*.md", "action": "READ for project context"}
}
```
...
| Input | Mode | Scope |
|-------|------|-------|
| `pff-test` (no args) | `diff` | Files changed in git diff |
| `pff-test feature N` | `feature` | Entire feature N scope |
| `pff-test <path>` | `path` | Specific directory/file |

SET `SCOPE_MODE` = determined mode.

### 2.2 Detect Existing Contract Tests (PRD0001 — TDD)

**BEFORE generating new tests, check for contract tests from `/pff-plan`:**

```bash
# Find existing contract test files (generated by test-spec subagent via /pff-dev)
...
5. INFORM user: "Detected [N] contract test files from /pff-plan. Will focus on gaps and edge cases."
...
IF no changed files:
  INFORM user: "No changed files detected. Specify a path or feature: `pff-test src/modules/auth` or `pff-test feature 1`"
  ⛔ STOP.

**IF MODE = feature:**
1. RUN: `bash .pff/scripts/feature-status.sh` (if exists)
...
1. READ .pff/skills/backend-development/SKILL.md (if exists) for project patterns
2. READ .pff/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .pff/skills/frontend-development/SKILL.md (if exists) for project patterns
2. READ .pff/project/*.md (if exists) for project conventions
...
## MANDATORY: Load Context
1. READ .pff/skills/backend-development/SKILL.md (if exists)
2. READ .pff/project/*.md (if exists)
...
IF `.pff/scripts/log-iteration.sh` exists:
```bash
bash .pff/scripts/log-iteration.sh "test" "pff-test" "Generated tests — [CURRENT_COVERAGE]% coverage" "[ALL_TEST_FILES comma-separated]"
```
...
- Regenerate contract tests that already exist from /pff-plan (detect and skip)
