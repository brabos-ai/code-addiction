...
**STEPS IN ORDER:**
```
STEP 1: Detect templates       → pff template status
STEP 2: Execute updates        → pff template update <slug> for each
...
IF TEMPLATES NOT DETECTED:
  ⛔ DO NOT USE: Bash for pff template update
  ⛔ DO NOT: Update templates
  ✅ DO: Execute pff template status FIRST
...
| Authentication error | Inform `pff login` and STOP |

**IF no templates installed:**
```
## PFF Update

No PFF templates installed in this project.

To install: `pff template install <slug>`
To list available: `pff template list`
```
→ STOP HERE.
...
**ANALYZE the message returned by `pff template update`:**
...
To keep: `git add . && git commit -m "chore: update pff templates"`
...
### DETECTION
- [ ] STEP 1: `pff template status` executed
- [ ] Templates identified (or informed that there are none)

### UPDATE
- [ ] STEP 2: `pff template update` executed for each template
...
| Error | Action |
|-------|--------|
| `pff` not found | Inform: "PFF CLI not installed. Execute: npm i -g @brabos-ai/pff-cli" |
| Not authenticated | Inform: "Execute `pff login` to authenticate" |
...
- Ignore the message returned by pff template update
