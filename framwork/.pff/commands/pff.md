# PFF - Intelligent Ecosystem Gateway

> **LANG:** Respond in user's native language (detect from input). Tech terms always in English. Documents in user's language.

Entry point for the pff-pro ecosystem. Answers questions, guides flow, suggests next command.
...
STEP 1: Check onboarding         → IF docs/owner.md does not exist → suggest /pff-init
...
```bash
cat .pff/skills/pff-ecosystem-map/SKILL.md
```

This file contains:
- All pff-pro commands with purpose and skills they load
...
**Use:** `/pff-init` to run onboarding (5-10 min)
...
```bash
cat .claude/commands/pff-[command].md
```
...
```bash
cat .pff/skills/dev-environment-setup/SKILL.md
```
...
|------------------|-------------------|-----------|
| No docs/owner.md | `/pff-init` | Project onboarding |
...
| Feature reviewed | `/pff-done` | Finalize and generate changelog |
| Epic with pending sub-features | `/pff-dev feature N` | Next sub-feature |
...
Or use `/pff [question]` for more guidance.
```
