...
if [ "$CREATE_PR" = false ]; then
    ...
    echo "COMMAND=bash .pff/scripts/feature-pr.sh --create-pr"
    exit 0
fi
...
Generated with PFF by https://brabos.ai

Co-Authored-By: PFF <noreply@brabos.ai>"
    echo "COMMIT=OK"
fi
...
echo ""
echo "NEXT_STEP=After PR is merged, run: bash .pff/scripts/feature-pr.sh --confirm-merge"
