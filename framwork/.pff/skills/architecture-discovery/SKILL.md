...
{"note":"Implementation details (logging, validation, state, styling) belong in .pff/project/*.md via /architecture-analyzer"}
...
{"purpose":"Link to .pff/project/*.md for implementation details","note":"CLAUDE.md = WHERE things are, .pff/project/*.md = HOW to implement"}
...
{"doNOT":["include logging patterns in CLAUDE.md","include validation patterns in CLAUDE.md","include state management details in CLAUDE.md","include styling patterns in CLAUDE.md","duplicate info already in .pff/project/*.md"]}
{"reference":"List ALL .pff/project/*.md files dynamically - don't hardcode names"}
...
### Implementation Patterns (if .pff/project/ exists)
{"note":"Detailed patterns documented separately for token efficiency"}
{"files":"List dynamically from .pff/project/*.md - named by path (APP-BACKEND.md, APP-ADMIN.md, LIB-DATABASE.md, etc)"}
...
- **Reference .pff/project/*.md for implementation patterns (if exist)**
...
- **Include implementation details in CLAUDE.md (logging, validation, state, styling) - these go in .pff/project/*.md**

**SEPARATION OF CONCERNS:**
- **CLAUDE.md** = WHERE things are (structure, paths, layers, packages)
- **.pff/project/*.md** = HOW to implement (patterns, conventions, examples)

---

## Workflow

1. Execute `bash .pff/scripts/architecture-discover.sh`
...
7. **Check if .pff/project/*.md exist** → add Implementation Patterns Reference section
8. Cleanup: `rm .claude/temp/architecture-discovery.md`
9. Report discoveries + suggest `/architecture-analyzer` if .pff/project/*.md don't exist
