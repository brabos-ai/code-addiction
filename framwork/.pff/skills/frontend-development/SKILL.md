...
{"rules":["interfaces not classes","Date→string","Enum→union type","no @pff/domain imports","sync with backend DTOs"]}
...
- [ ] UX design skill loaded if no `design.md`
  → Check: `.pff/skills/ux-design/SKILL.md` read when design.md absent
...
### Build
- [ ] Build passes: `npm run build -w @pff/frontend`
