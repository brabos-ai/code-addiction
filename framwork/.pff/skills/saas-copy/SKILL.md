...
**Não use para:** Executar workflow (use comando `/pff-copy`)

**Relação:**
- `saas-copy` (skill) = Conhecimento, frameworks, templates
- `/pff-copy` (comando) = Execução do workflow + geração de arquivos
...
- Para executar workflow completo → use `/pff-copy`
...
O comando `/pff-copy` usa estas fontes para análise automática:
...
Perguntas que o comando `/pff-copy` faz ao usuário:
