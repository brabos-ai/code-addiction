...
Dispatch review subagent with this prompt:
    ...
    ## SKILLS
    - .pff/skills/code-review/SKILL.md
...
**Reference scripts:**
- `bash .pff/scripts/feature-status.sh` - Get feature context
- `bash .pff/scripts/architecture-discover.sh` - Codebase structure overview

**Skills to compose:**
- Backend: `.pff/skills/backend-development/SKILL.md`
- Database: `.pff/skills/database-development/SKILL.md`
- Frontend: `.pff/skills/frontend-development/SKILL.md` + `.pff/skills/ux-design/SKILL.md`
- Review: `.pff/skills/code-review/SKILL.md`
...
```bash
bash .pff/scripts/log-jsonl.sh "docs/features/${FEATURE_ID}/decisions.jsonl" "<type>" "<agent>" '"decision":"[what]","reason":"[why]"'
```

### Central File (consolidated by /pff-done)

`.pff/project/decisions.jsonl` — all project decisions, consolidated at feature completion.
...
After plan.md is created and validated (STEP 9.4 of /pff-plan).
...
### TASKS MODE in /pff-dev

When `HAS_TASKS=true`, /pff-dev executes by tasks:
