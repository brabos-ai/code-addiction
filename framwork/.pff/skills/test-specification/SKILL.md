...
{"trigger":"subagent test-spec no /pff-plan","input":["about.md (RFs, RNs, RNFs)","plan-database.md","plan-backend.md","plan-frontend.md"],"output":"plan-test-spec.md","focus":"contract tests derivados de requisitos","complements":"pff-test (post-dev coverage)"}

---

## Quando Usar

- Subagente test-spec do `/pff-plan` (automático)
- Sempre que `about.md` tiver RFs/RNs e contratos técnicos existirem

## Quando NÃO Usar

- Para gerar testes de cobertura pós-implementação (usar `/pff-test`)
- Para edge cases e error handling (complemento do `/pff-test`)
