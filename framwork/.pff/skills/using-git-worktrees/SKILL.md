...
2. Execute os comandos de desenvolvimento lá:
   ...
   - `/pff-done` - para finalizar

### Neste VSCode (atual):
- Você pode continuar trabalhando em outras coisas
- A worktree é independente - não afeta este workspace
- Quando terminar a feature, a branch será mergeada via `/pff-done` no outro VSCode
...
### No novo VSCode:
1. O Claude Code terá contexto isolado da worktree
2. Execute `/plan`, `/dev`, `/autopilot`, ou `/pff-done` lá
...
- `/pff-done` → Finaliza e faz merge (executado no VSCode da worktree)
