...
- `layer`: **derived from the items, never hardcoded.** An item whose `at` sits under `framwork/`,
  `framwork/provider-map.json`, `cli/` or `mcp/` is product; everything else is internal. The entry takes
...
⛔ **Write this delivery's own entry FIRST, then each `superseded` line from 3.3.** A `superseded` line's
`superseded_by` must name an id the index already holds, so a supersession written before the entry it
points at is refused with `REFUSED=superseded-by-unknown`.
...
⛔ **`find` is one contiguous token, byte-exact, and never anchored into `docs/` or into a path the project ignores.** A multi-word anchor is what a formatter wraps, and a wrapped anchor silently flips a live item to `gone`. **A `superseded` line copies its old items as they are**: the script skips the checks that read the repository for it — `find` absent, over-matched, or `at` now ignored — and keeps the `docs/` and whitespace checks.
