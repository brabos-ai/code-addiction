...
- An F-block naming `workbench/`, `scripts/` or the repo root — **`mcp/` excepted**, which sits at
  the root and is product because `scripts/build.js` copies it into the npm package.

**This skill does not write `AGENTS.md`.** Its inventory block is generated by
...
- Edit `AGENTS.md` — its inventory block is generated and the rest changes only through a plan
